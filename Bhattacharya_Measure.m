...
%%% Bhattacharya Measure algorithm %%%
...
% Initializing 
BM_Result=zeros(size(Probab_C,2),NS); %  no. of stns x NS sources
...
while k<=size(Probab_S,2) % Loop for sources
    for j=1:C_cols % Loop for stations
        ...
        BM_Result(j,k)=sum(sqrt(Probab_C(:,j).*Probab_S(:,k)));
...
k=k+1;
    ...
    xlswrite('Bhattacharya_Result.xlsx',BM_Result,sheetname1);